...
impl futhark_u8_2d {
   unsafe fn new<C>(ctx: C, arr: &[u8], dim: &[i64]) -> *const Self
   where C: Into<*mut bindings::futhark_context>
   {
     let ctx = ctx.into();
     bindings::futhark_new_u8_2d(
       ctx,
       arr.as_ptr() as *mut u8,
       dim[0],
dim[1],
)
     }
...
impl FutharkType for futhark_u8_2d {
   type RustType = u8;
   const DIM: usize = 2;

    unsafe fn shape<C>(ctx: C, ptr: *const bindings::futhark_u8_2d) -> *const i64
    where C: Into<*mut bindings::futhark_context>
    {
        let ctx = ctx.into();
        bindings::futhark_shape_u8_2d(ctx, ptr as *mut bindings::futhark_u8_2d)
    }
    unsafe fn values<C>(ctx: C, ptr: *mut Self, dst: *mut Self::RustType) -> Result<()>
    where C: Into<*mut bindings::futhark_context>
    {
        let ctx = ctx.into();
        let ret = bindings::futhark_values_u8_2d(ctx, ptr, dst);
        if ret != 0 {
            return Err(crate::FutharkError::new(ctx).into());
        } 
        // Sync the values to the array.
    ...
    {
        let ctx = ctx.into();
        bindings::futhark_free_u8_2d(ctx, ptr)
    }}
...
impl FutharkType for futhark_u8_3d {
   type RustType = u8;
   const DIM: usize = 3;

    unsafe fn shape<C>(ctx: C, ptr: *const bindings::futhark_u8_3d) -> *const i64
    where C: Into<*mut bindings::futhark_context>
    {
        let ctx = ctx.into();
        bindings::futhark_shape_u8_3d(ctx, ptr as *mut bindings::futhark_u8_3d)
    }
    unsafe fn values<C>(ctx: C, ptr: *mut Self, dst: *mut Self::RustType) -> Result<()>
    where C: Into<*mut bindings::futhark_context>
    {
        let ctx = ctx.into();
        let ret = bindings::futhark_values_u8_3d(ctx, ptr, dst);
        if ret != 0 {
            return Err(crate::FutharkError::new(ctx).into());
        } 
        // Sync the values to the array.
    ...
    {
        let ctx = ctx.into();
        bindings::futhark_free_u8_3d(ctx, ptr)
    }}
#[derive(Debug)]
...
impl Array_i64_1d {
    ...
    pub fn to_vec(&self) -> Result<(Vec<i64>, Vec<i64>)>
    {
        ...
        unsafe {
            ...
            futhark_i64_1d::values(ctx, self.as_raw_mut(), buffer.as_mut_ptr())?;
            Ok((buffer, shape))
        }
    }
...
impl Array_u64_2d {
    ...
    pub fn to_vec(&self) -> Result<(Vec<u64>, Vec<i64>)>
    {
        ...
        unsafe {
            ...
            futhark_u64_2d::values(ctx, self.as_raw_mut(), buffer.as_mut_ptr())?;
            Ok((buffer, shape))
        }
    }
...
impl Array_u64_3d {
    ...
    pub fn to_vec(&self) -> Result<(Vec<u64>, Vec<i64>)>
    {
        ...
        unsafe {
            ...
            let mut buffer: Vec<u64> =
                vec![u64::default(); elems];
            futhark_u64_3d::values(ctx, self.as_raw_mut(), buffer.as_mut_ptr())?;
            Ok((buffer, shape))
        }
    }
...
#[derive(Debug)]
pub struct Array_u8_2d {
    ptr: *const futhark_u8_2d,
    ctx: *mut bindings::futhark_context,
}

impl Array_u8_2d {
    pub(crate) unsafe fn as_raw(&self) -> *const futhark_u8_2d {
         self.ptr
    }

    pub(crate) unsafe fn as_raw_mut(&self) -> *mut futhark_u8_2d {
         self.ptr as *mut futhark_u8_2d
    }
    pub(crate) unsafe fn from_ptr<T>(ctx: T, ptr: *const futhark_u8_2d) -> Self
        where
        T: Into<*mut bindings::futhark_context>,
    {
        let ctx = ctx.into();
    ...
    pub(crate) unsafe fn shape<T>(ctx: T, ptr: *const futhark_u8_2d) -> Vec<i64>
    where
        T: Into<*mut bindings::futhark_context>,
    {
        let ctx = ctx.into();
        let shape_ptr: *const i64 = futhark_u8_2d::shape(ctx, ptr);
        let shape = std::slice::from_raw_parts(shape_ptr, 2);
        Vec::from(shape)
    }

    pub fn from_vec<T>(ctx: T, arr: &[u8], dim: &[i64]) -> Result<Self>
    where
        T: Into<*mut bindings::futhark_context>,
    {
        let expected = (dim.iter().fold(1, |acc, e| acc * e)) as usize;
        ...
        let ctx = ctx.into();
        unsafe {
            let ptr = futhark_u8_2d::new(ctx, arr, dim);
            Ok(Array_u8_2d { ptr, ctx })
        }
    }
    
    pub fn to_vec(&self) -> Result<(Vec<u8>, Vec<i64>)>
    {
        let ctx = self.ctx;
        unsafe {
            futhark_context_sync(ctx);
            ...
            let mut buffer: Vec<u8> =
                vec![u8::default(); elems];
            futhark_u8_2d::values(ctx, self.as_raw_mut(), buffer.as_mut_ptr())?;
            Ok((buffer, shape))
        }
    }
    ...
    pub(crate) unsafe fn free_array(&mut self)
    {
        if futhark_u8_2d::free(self.ctx, self.as_raw_mut()) != 0 {
            panic!("Deallocation of object failed, this should not happen \
                    outside of compiler bugs and driver or hardware malfunction.");
        }
    }
...
#[derive(Debug)]
pub struct Array_u8_3d {
    ptr: *const futhark_u8_3d,
    ctx: *mut bindings::futhark_context,
}

impl Array_u8_3d {
    pub(crate) unsafe fn as_raw(&self) -> *const futhark_u8_3d {
         self.ptr
    }

    pub(crate) unsafe fn as_raw_mut(&self) -> *mut futhark_u8_3d {
         self.ptr as *mut futhark_u8_3d
    }
    pub(crate) unsafe fn from_ptr<T>(ctx: T, ptr: *const futhark_u8_3d) -> Self
        where
        T: Into<*mut bindings::futhark_context>,
    {
        let ctx = ctx.into();
    ...
    pub(crate) unsafe fn shape<T>(ctx: T, ptr: *const futhark_u8_3d) -> Vec<i64>
    where
        T: Into<*mut bindings::futhark_context>,
    {
        let ctx = ctx.into();
        let shape_ptr: *const i64 = futhark_u8_3d::shape(ctx, ptr);
        let shape = std::slice::from_raw_parts(shape_ptr, 3);
        Vec::from(shape)
    }
        ...
        T: Into<*mut bindings::futhark_context>,
    {
        ...
        let ctx = ctx.into();
        unsafe {
            let ptr = futhark_u8_3d::new(ctx, arr, dim);
            Ok(Array_u8_3d { ptr, ctx })
        }
    }
    ...
    pub fn to_vec(&self) -> Result<(Vec<u8>, Vec<i64>)>
    {
        ...
        unsafe {
            ...
            let mut buffer: Vec<u8> =
                vec![u8::default(); elems];
            futhark_u8_3d::values(ctx, self.as_raw_mut(), buffer.as_mut_ptr())?;
            Ok((buffer, shape))
        }
    }
    ...
    pub(crate) unsafe fn free_array(&mut self)
    {
        if futhark_u8_3d::free(self.ctx, self.as_raw_mut()) != 0 {
            panic!("Deallocation of object failed, this should not happen \
                    outside of compiler bugs and driver or hardware malfunction.");
        }
    }
